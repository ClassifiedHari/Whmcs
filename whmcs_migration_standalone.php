<?php
/**
 * WHMCS 8.11.2 to Custom Admin System Migration Script
 * 
...
class WhmcsMigration {
    ...
    private function migrateClients() {
        ...
        while ($offset < $total) {
            $stmt = $this->whmcs_pdo->prepare(
                "SELECT * FROM tblclients ORDER BY id LIMIT ? OFFSET ?"
            );
            $stmt->execute([$this->settings['batch_size'], $offset]);
            ...
            $offset += $this->settings['batch_size'];
    ...
    private function migrateDomains() {
        ...
        while ($offset < $total) {
            $stmt = $this->whmcs_pdo->prepare(
                "SELECT * FROM tbldomains ORDER BY id LIMIT ? OFFSET ?"
            );
            $stmt->execute([$this->settings['batch_size'], $offset]);
    ...
    private function migrateInvoices() {
        ...
        while ($offset < $total) {
            $stmt = $this->whmcs_pdo->prepare(
                "SELECT * FROM tblinvoices ORDER BY id LIMIT ? OFFSET ?"
            );
            $stmt->execute([$this->settings['batch_size'], $offset]);
    ...
    private function migrateTicketReplies() {
        ...
        while ($offset < $total) {
            $stmt = $this->whmcs_pdo->prepare(
                "SELECT * FROM tblticketreplies ORDER BY id LIMIT ? OFFSET ?"
            );
            $stmt->execute([$this->settings['batch_size'], $offset]);
